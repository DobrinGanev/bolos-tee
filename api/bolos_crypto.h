 ...
 * @param  [in,out] hash
 *   Hash context
 *   The hash context pointer shall point to  either a bls_ripemd160_t, either a bls_sha256_t  or bls_sha512_t .
 ...
 *
 * @return 1 if success, not finished. Hash size if success, finished. 0 if error
 */
int bls_hash(bls_hash_t *hash, int mode,
            const uint8_t WIDE *in, size_t len, uint8_t *out);
 ...
 * @param [in,out] hmac
 *   Hmac context
 *   The hmac context pointer shall point to  either a bls_ripemd160_t, either a bls_sha256_t  or bls_sha512_t .
 ...
 * @param [in] mode 
 *   16bits flags. See Above
 *   If BLS_LAST is set and BLS_DISCARD is not set, context is automatically re-inited.
 *   Supported flags:
 *     - BLS_LAST
 *     - BLS_DISCARD
 *
 ...
 *   'out' length is implicit, no check is done
 *
 * @return 1 if success, not finished. Hmac size if success, finished. 0 if error
 */
int bls_hmac(bls_hmac_t *hmac, int mode,
            const uint8_t WIDE *in, size_t len, uint8_t *mac);
 ...
 */
int bls_des(bls_des_key_t WIDE *key, int mode,
 ...
 * @throws INVALID_PARAMETER
 */
int bls_des_iv(bls_des_key_t WIDE *key, int mode,
              const bls_area_t *iv,
              const bls_area_t *in, bls_area_t *out);
 ...
 * @throws INVALID_PARAMETER
 */
int bls_aes(bls_aes_key_t WIDE *key, int mode,
           const bls_area_t *in, bls_area_t *out);
 ...
 *
  * @param [in] iv
 *   Initialization Vector and associated length
 *   Only meaningful for the first block of a BLS_CHAIN_CBC, BLS_AES_PAD_CTR, BLS_AES_PAD_CFB, BLS_AES_PAD_OFB chaining. 
 *   'len' shall be a multiple of DES_BLOCK_SIZE.
 *
 * @param [in] in
 *   Input data to encrypt/decrypt and associated length
 ...
 * @throws INVALID_PARAMETER
 */
int bls_aes_iv(bls_aes_key_t WIDE *key, int mode,
              const bls_area_t *iv,
              const bls_area_t *in, bls_area_t *out);
 ...
 * @throws INVALID_PARAMETER
 */
int bls_aes_iv_gcm(bls_aes_key_t WIDE *key, int mode,
              const bls_area_t *in,
              const bls_area_t *iv,
              const bls_area_t WIDE *aadTag, // encrypt : aad in/tag out | decrypt : tag in
              bls_area_t *out);
 ...
 *   Key parameters shall include the modulus and public exponent encoded as big endian raw value
 *
 * @param [out] key
 ...
 * @param [in] keyData
 *   Key parameters value or NULL.
 *   Key parameters shall include the modulus and private exponent encoded as big endian raw value
 *
 * @param [out] key
 ...
 * @param [in] keyData
 *   Key parameters value or NULL.
 *   Key parameters shall include Q, P, DP, DQ, QInv encoded as big endian raw value
 *
 * @param [out] key
 *   Public RSA key to initialize.
 *
 ...
 *
 * @param [in,out] dest
 *   Destination buffer and length. Length is modified by ENCRYPT, DECRYPT, SIGN operations
 *
 * @return 
...
int bls_ecdsa_init_public_key(bls_curve_t curve, 
                             const uint8_t  WIDE *rawkey, size_t key_len,
...
int bls_ecfp_generate_pair(bls_curve_t curve,
                          bls_ecfp_public_key_t  *public_key, bls_ecfp_private_key_t *private_key, 
                          uint8_t *d);
 ...
 *
 * @param [in] hashID
 *  Hash to use for nonce generation when using BLS_RND_RFC6979 (shall be BLS_SHA256)
 *
 * @param [in] hash
 *   Input data to sign. 
 ...
 * @throws INVALID_PARAMETER
 */
int bls_schnorr_sign(bls_ecfp_private_key_t WIDE *key, int mode, bls_md_t hashID,
                  const uint8_t WIDE *hash, size_t hash_len, uint8_t *sig);
/**
 ...
 * @throws INVALID_PARAMETER
 */
int bls_schnorr_verify(bls_ecfp_public_key_t WIDE *key, int mode, bls_md_t hashID,
                  const uint8_t WIDE *hash, size_t hash_len, uint8_t *sig);
 ...
 *
 */
void bls_math_modm(uint8_t *v, size_t len_v,
                  const uint8_t WIDE *m, size_t len_m);
