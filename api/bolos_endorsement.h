 ...
 *
 * The endorsement API let developers verify cryptographic proofs that the code is running on a trusted
 * device initialized by the issuer
 *
 * Two set of algorithms are available :
 *    - Algorithm 1 provides a secret to the running code, and let the running code sign a message. 
 *      Both elements are bound to the running code and the unique per device private key A1.
 *
 *    - Algorithm 2 let the running code sign a message using a key derived from the unique per device 
 *    private key A2
 *
...
/**
 * @brief Get the per device unique uncompressed public key signing the endorsement initialization request
 *
 * @param [out] out
 ...
 *
 * When called, the device generates a secp256k1 new key pair for the given endorsement key
 * then returns the uncompressed public key and a signature of the uncompressed public key by 
...
/**
 * @brief Finalize an endorsement key provisioning process by providing the associated certificate
 *
 ...
 *
 * The application secret is defined as a HMAC-SHA512 of the currently running code hash using the
 ...
 * by a private key computed by adding mod n the HMAC-SHA256 of the endorsement key 2 public key 
 * using the currently running code hash as key to the endorsement key 2 private key.
 *
 * @param [in] in
 *   Buffer containing the message to sign
 *
