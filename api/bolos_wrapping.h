...
/** 
 * @brief Bind data to a specific device, device session, and running application
 * @file bolos_wrapping.h
 * @author Ledger Firmware Team <hello@ledger.fr>
 * @version 1.0
 * @date 29th of February 2016
 *
 * The wrapping API allows executed code to exchange secret blobs with the Normal World to be 
 * consumed later.
 *
...
enum bls_wrapping_scope_e {
  ...
  BLS_SCOPE_APPLICATION, /**< only the creating application can access on this device */
  BLS_SCOPE_SESSION, /**< all applications can access on this device for this session */
  BLS_SCOPE_SESSION_APPLICATION, /**< only the creating application can access on this device for this session */
  BLS_SCOPE_PERSONALIZATION /**< can only unwrap a blob sent by the issuer server */
};
...
unsigned int bls_wrap(bls_wrapping_scope_t scope,
	              const uint8_t WIDE* in, size_t length,
 ...
 * @throws INVALID_PARAMETER
 */
unsigned int bls_unwrap(bls_wrapping_scope_t scope, const uint8_t WIDE *in, size_t length,
	              uint8_t *out, size_t outLength);
