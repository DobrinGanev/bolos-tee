...
/** 
 * @brief Interface to the main Bitcoin wallet provided to third party applications
 * @file bolos_wallet.h
 * @author Ledger Firmware Team <hello@ledger.fr>
 * @version 1.0
 * @date 29th of February 2016
 *
 * Using this API third party applications can interact with the Bitcoin wallet or 
 ...
 *
 * @param [in] details
 *   Derivation method, either BLS_WALLET_DERIVE_PUBLIC or BLS_WALLET_DERIVE_PRIVATE
 *
 * @param [in] path
 ...
 *   Pointer to a previously initialized private key to store the derived private key
 *
 * @param [out] publicKey
 *   Pointer to a previously initialized public key to store the derived public key
 *
 * @return 1 if success, 0 if error
 * 
 */
int bls_wallet_derive(uint8_t details,
	     const uint32_t WIDE* path, size_t pathLength,
	     uint8_t *chainCode,
 ...
 *
 * @warning In the current API release, the version of the wallet application is used for the address version.
 ...
 *
 * @param [in] compressed
 *   Compute the address for a compressed key if set to true, otherwise compute it for an uncompressed key
 * 
 * @return address length if success, 0 if error
 * 
 */
int bls_wallet_get_address(bls_ecfp_public_key_t *publicKey,
	     char WIDE* address, size_t addressLength,
	     bool compressed);
 ...
 * The Ledger Wallet application is called in a special mode where the Trusted UI is disabled
 *
 * @param [in] apdu
 *   APDU buffer containing the command to send to the Ledger Wallet application
 *   This buffer is overwritten by the response
 *
 * @return size of the response data and 2 bytes Status Word if success, 0 if error
 * 
 */
