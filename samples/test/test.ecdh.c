...
int main(int argc, char **argv) {
	...
	for (int i=0; i<2; i++) {
		...
		bls_ecdh(&privateKey, BLS_ECDH_POINT, publicPoint, secret);
                ...
                bls_ecdh(&privateKey2, BLS_ECDH_POINT, publicPoint, secret2);
		if (compare(secret, secret2, 65)) {
			...
			bls_debug("ECDH point not ok");
                ...
                bls_ecfp_get_public_component(&publicKey, publicPoint);
                bls_ecdh(&privateKey2, BLS_ECDH_X, publicPoint, secret2);
                if (compare(secret, secret2, 32)) {
                        bls_debug("ECDH X ok");
                }
                else {
		...
		if (curve == BLS_CURVE_256K1) {
                	...
                	bls_ecfp_get_public_component(&publicKey, publicPoint);
                	bls_ecdh(&privateKey2, BLS_ECDH_HASHED, publicPoint, secret2);
                	if (compare(secret, secret2, 32)) {
